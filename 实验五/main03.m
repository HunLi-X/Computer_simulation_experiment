...
%% 定义连续系统传递函数
num = [-2, 3, -4];                   % 分子系数
den = [1, 3.2, 1.61, 3.03];         % 分母系数
Gc = tf(num, den);                   % 连续传递函数
...
% 连续系统阶跃响应（作为参考）
t_cont = 0:0.001:10;
[y_cont, t_cont] = step(Gc, t_cont);
plot(t_cont, y_cont, 'k-', 'LineWidth', 2.5); hold on;

% 对每个采样周期进行零阶保持器（ZOH）离散化
for k = 1:length(T_values)
    T = T_values(k);
    Gd = c2d(Gc, T, 'zoh');          % 零阶保持器离散化
    ...
    % 离散系统阶跃响应
    [y_d, t_d] = step(Gd, 10);
    stem(t_d, y_d, colors{k}, 'Marker', 'none', ...
         'LineWidth', 1.5, 'LineStyle', line_styles{k});
end

xlabel('时间 t / s', 'FontSize', 12);
.ylabel('输出 y(t)', 'FontSize', 12);
title('连续系统与不同采样周期离散化系统的阶跃响应对比', 'FontSize', 14);
legend({'连续系统', 'T=0.01s (ZOH)', 'T=0.1s (ZOH)', 'T=1s (ZOH)'}, ...
...
for k = 1:length(T_values)
    ...
    % 计算误差
    error = y_d(:) - y_d_interp(:);

    subplot(length(T_values), 1, k);
    bar(t_d, error, colors{k});
    xlabel('时间 t / s');
    ylabel('误差');
    title(sprintf('T = %.3f s 时的离散化误差 (最大误差 = %.4f)', T, max(abs(error))));
    grid on;
end
sgtitle('不同采样周期下的离散化误差', 'FontSize', 13, 'FontWeight', 'bold');

%% ============ 图3：极点位置对比（z平面） ============
figure('Name', 'z平面极点对比', 'Position', [50 50 800 400]);

theta = 0:0.01:2*pi;
plot(cos(theta), sin(theta), 'k--', 'LineWidth', 1); hold on;  % 单位圆
...
for k = 1:length(T_values)
    ...
    Gd = c2d(Gc, T, 'zoh');
    p = pole(Gd);
    plot(real(p), imag(p), markers{k}, 'Color', colors{k}, ...
